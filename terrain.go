	...
	placeEntities := func(room Rect) {
		...
		for i := 0; i < monsterCount; i++ {
			...
			if !e.FoundAtPosition(location) {
				var monster *Entity
				if rand.Intn(100) < 80 {
					monster = NewEntity(location, 'o', "Orc", "AnsiLightGreen", true, &MonsterBrain{}, NewFighter(10, 0, 3), RoActor, EtMonster)
				} else {
					monster = NewEntity(location, 'T', "Troll", "AnsiGreen", true, &MonsterBrain{}, NewFighter(16, 1, 4), RoActor, EtMonster)
				}
				e.Append(monster)
			}
		}
